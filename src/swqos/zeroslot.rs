        ...
        if response_json.get("result").is_some() {
            ...
            match crate::swqos::common::poll_transaction_confirmation(&self.rpc_client, signature).await {
                Ok(transaction_result) => {
                    println!(" 0slot{} confirmation: {:?}", trade_type, start_time.elapsed());
                    Ok(transaction_result)
                }
                Err(e) => {
                    eprintln!(" 0slot{} confirmation failed: {:?}", trade_type, e);
                    Err(e)
                }
            }
        } else if let Some(_error) = response_json.get("error") {
            eprintln!(" 0slot{} submission failed: {:?}", trade_type, _error);
            Err(anyhow::anyhow!("0slot submission failed: {:?}", _error))
        } else {
            eprintln!(" 0slot{} unexpected response format: {}", trade_type, response_text);
            Err(anyhow::anyhow!("0slot unexpected response format: {}", response_text))
        }
    }
